/*
  # Tabelas de Configuração do DRE
  ...
  1. Novas Tabelas
    - `dre_config_accounts`
      ...
      - category_ids (array de uuid, para tipo category)
      - indicator_id (uuid, para tipo indicator)
      - selected_accounts (array de uuid, para tipo total)
      - parent_account_id (auto-referência)
      - is_active (booleano)
      - sign (enum: positive, negative, para tipo blank)
...
-- Função para gerar código da conta
CREATE OR REPLACE FUNCTION generate_dre_config_account_code()
RETURNS text AS $$
DECLARE
  next_number integer;
  next_code text;
...
CREATE OR REPLACE FUNCTION set_dre_config_account_code()
RETURNS trigger AS $$
BEGIN
  IF NEW.code IS NULL THEN
    NEW.code := generate_dre_config_account_code();
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_set_dre_config_account_code
  BEFORE INSERT ON dre_config_accounts
  FOR EACH ROW
  EXECUTE FUNCTION set_dre_config_account_code();
